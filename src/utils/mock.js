export default (column, dic, defaultForm, run) => {
  ...
  function createDic({ dicData, valueKey, columnType, multiple }) {
    ...
    if (['checkbox'].includes(columnType) || multiple) {
      ...
      for (let i = 0; i < number; i++) {
        ...
        while (flag) {
          ...
          const values = dicData[numbers][valueKey];
      ...
      return dicData[number][valueKey];
    }
  }
  ...
  function create() {
    Object.keys(column).forEach(index => {
      ...
      if (ele.mock && typeof ele.mock === 'object') {
        let params = ele.mock;
        params.dicData = typeof ele.dicName === 'string' ? dic[ele.dicName] : (typeof ele.options === 'function' ? ele.options({ row: defaultForm }) : ele.options) || [];
        params.valueKey = ele.valueKey || 'code';
